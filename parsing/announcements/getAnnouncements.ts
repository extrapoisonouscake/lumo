...
export async function getAnnouncements(school: KnownSchools, date?: Date) {
  const redisKey = `announcements:${school}:${timezonedDayJS(date).format(
    DATE_FORMAT
  )}`;
  ...
  const preparedData = parseElements(parsedPDF);
...
async function parsePDF(buffer: ArrayBuffer) {
  ...
  return elements as PDFParsingPartitionElement[];
}
